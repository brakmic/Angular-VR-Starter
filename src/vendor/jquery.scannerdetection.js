...
module.exports = (function() {
    ...
    window.window.$.fn.scannerDetection=function(options){
	...
	if(options === false){
        ...
        if(typeof options!=="object"){
            options=window.$.extend({},defaults);
        }else{
            options=window.$.extend({},defaults,options);
        }

        this.each(function(){
            var self=this, $self=window.$(self), firstCharTime=0, lastCharTime=0, stringWriting='', callIsScanner=false, testTimer=false, scanButtonCounter=0;
		...
		if(typeof options.ignoreIfFocusOn === 'string') return window.$(':focus').is(options.ignoreIfFocusOn);
	        if(typeof options.ignoreIfFocusOn === 'object' && options.ignoreIfFocusOn.length){
		    var focused=window.$(':focus');
